		...
		<?php
			...
			if(mysqli_num_rows($res))
			{
				while($row=mysqli_fetch_assoc($res))
				{
					...
					if(isset($_POST["box$sid"]))
					{
						mysqli_query($conn,"insert into payments values('$sid',$month)");
					}
					else
						mysqli_query($conn,"delete from payments where studentId=$sid");
				}
			}
			}
			?>
				...
				<div class="header_bottom">
					<nav>
						<ul id="nav">
							...
							<li id="dropdown"><a href="">Add User</a>
								<ul>
			...
			<div class="clearfix wrapper main_content_area">
				<div class="clearfix main_content floatleft">
					<div class="clearfix content">
						...
						<?php
							$month=date('n');
							$querygetPayments="select 
												student.studentId,student.studentName,'Payed'=
															case
																when student.studentId in (select payments.studentId from payments where payments.month=$month) then 1
																else 0
															end
														from student";
							if(!($res=mysqli_query($conn,$querygetPayments)))
							...
							else
							{
								if(mysqli_num_rows($res)>0)
								{
									...
									echo "<table><th>StudentId</th><th>Student Name</th><th>Payed</th>";
									echo "</table></div>";
									...
									while($row=mysqli_fetch_array($res))
									{
										...
										echo "<tr>";
										if($payed==0)
											echo "<td>".$row['studentId']."</td><td>".$row['studentName']."</td><td><input type=checkbox checked=checked name=box$stdId></td>";
										else
											echo "<td>".$row['studentId']."</td><td>".$row['studentName']."</td><td><input type=checkbox name=box$stdId></td>";
										echo "</tr>";
									}
									echo "</table></div><br/>";
								}
